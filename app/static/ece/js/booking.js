...
function openAddRoom() {
  ...
  // ล้างไฟล์รูปภาพที่เคยค้างในอินพุตอัปโหลด
...
function openEditRoom(roomId) {
  ...
  // ล้างไฟล์รูปภาพในอินพุตอัปโหลด (กรณีต้องการเลือกรูปใหม่มาทดแทน)
  const imgEl = document.getElementById("rmImage");
  if (imgEl) imgEl.value = "";
...
async function saveRoom() {
  ...
  const seats = document.getElementById("rmSeats").value;
  ...
  formData.append("capacity", Number(seats) || 0);
  formData.append("is_active", isActive);

  // แนบรูปภาพในกรณีที่มีการเลือกไฟล์ใหม่
  ...
  try {
    if (editRoomId) {
      await api.patch(`/api/admin/req/room/${editRoomId}/`, formData);
      showToast("แก้ไขห้องเรียบร้อยแล้ว", "check_circle");
    } else {
      await api.post("/api/admin/req/room/", formData);
